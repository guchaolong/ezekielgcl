...
参考：
[GitHub - biliup/biliup: 全自动录播、直播录制、分p投稿工具，支持twitch、ytb频道搬运。](https://github.com/biliup/biliup)
[Centos部署biliup自动录播程序|分享即记忆](https://blog.waitsaber.org/archives/163)
...
title = "{title}%Y-%m-%d | 录播录像" ### 自定义标题的时间格式, {title}代表当场直播间标题 {streamer}代表在本config里面设置的主播名称 {url}代表设置的该主播的第一条直播间链接
...
tags = [ "游戏", "电子竞技", "英雄联盟", "直播", "卡尔", "录播", "直播回放", "卡尔直播回放", "卡尔直播录像"]
...
使用`biliup start`启动，Linux下以daemon进程启动，录像和日志文件保存在执行目录下，程序执行过程可查看日志文件。启动之后使用命令`ps -A | grep biliup` 查看进程biliup是否启动成功
...
# 实操
完成以上的步骤后
进入到 biliup 目录，这个目录下有 config.toml文件和 cookies.json文件了
`biliup start`开始运行
`ls`看一下，如果开播了，就会有相关的 xxxx.part文件
关播之后就会自动上传到 B 站了

怎么测试是否能成功上传？
`biliup restart`重新启动，之前录制的就会上传，去B站创作中心检查（比较慢，要等一会）
