...
# 2.桥接：
一个对象存在<mark style="background: #FF5582A6;">多个维度在变化</mark>，将各个维度分离开来，每个维度都可以独立扩展，<mark style="background: #FF5582A6;">聚合</mark>
...
# 7.代理：
客户不想或<mark style="background: #FF5582A6;">不能直接引用</mark>一个<mark style="background: #FF5582A6;">对象</mark>，可以通过一个称之为“<mark style="background: #FF5582A6;">代理</mark>”的<mark style="background: #FF5582A6;">第三者</mark>来实现<mark style="background: #FF5582A6;">间接引用</mark>
...
# 1.职责链
过把请求从一个对象传递到链条中下一个对象的方式来解除对象之间的耦合，直到请求被处理完毕。链中的对象是同一接口或抽象类的不同实现
...
提供对象的状态恢复机制
在不破坏封装的前提下，捕获一个对象的<mark style="background: #FF5582A6;">内部状态</mark>，并在该对象之外保存这个状态，这样可以在以后将对象恢复到原先保存的状态

# 7.观察者
定义对象间的一种<mark style="background: #FF5582A6;">一对多</mark>依赖关系，使得每当一个对象状态发生改变时，其相关依赖对象皆得到通知并自动更新
