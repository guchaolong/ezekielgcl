...
- 使用
  ...
  - SpringMVC中的DispatcherServlet
    ...
    有了适配器，把具体的处理器适配成HandlerAdapter，都以handle(request, response, handler)的方式调用，以上处理器的适配器分别对应HttpRequestHandlerAdapter、SimpleServletHandlerAdapter、SimpleControllerHandlerAdapter，它们三个都实现了 HandlerAdapter，**这里的HandlerAdapter其实就是类图中的Target**
